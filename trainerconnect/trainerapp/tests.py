...
def test_update_exercise_view(user, client, exercise):
    ...
    response = client.post(
        reverse("update-exercise", kwargs={"pk": exercise.id}), data=payload
    )
    assert response.status_code == 302
...
def test_add_training_view(user, client, exercise):
    ...
    response = client.post(
        "/add_training/",
        {"name": "Nowy trening", "description": 5, "exercises": exercise.id},
    )
    assert response.status_code == 302
...
def test_training_plan_detail_view_requires_login(user, client, training_plan):
    response = client.get(
        reverse("training-plan-details", kwargs={"pk": training_plan.id})
    )
    assert response.status_code == 302
    client.force_login(user=user)
    response = client.get(
...
def test_update_training_plan_view_requires_login(user, client, training_plan):
    response = client.get(
        reverse("update-training-plan", kwargs={"pk": training_plan.id})
    )
    assert response.status_code == 302
    client.force_login(user=user)
    response = client.post(
...
def test_update_training_plan_view(user, client, training, training_plan):
    ...
    response = client.post(
        reverse("update-training-plan", kwargs={"pk": training_plan.id}), data=payload
    )
    assert response.status_code == 302
    training_plan.refresh_from_db()
    assert training_plan.name == payload["name"]
    assert training_plan.description == payload["description"]
    assert list(training_plan.trainings.values_list("id", flat=True)) == [
        payload["trainings"]
    ]
...
def test_update_training_view(user, client, exercise, training):
    ...
    response = client.post(
        reverse("update-training", kwargs={"pk": training.id}), data=payload
    )
    assert response.status_code == 302
    print(response.url)
    ...
    assert list(training.exercises.values_list("id", flat=True)) == [
        payload["exercises"]
    ]
...
def test_delete_training_plan_view_requires_login(user, client, training_plan):
    response = client.get(
        reverse("delete-training-plan", kwargs={"pk": training_plan.id})
    )
    assert response.status_code == 302
    client.force_login(user=user)
    response = client.post(
